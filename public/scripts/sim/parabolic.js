...
export function createSimulation({ canvas, ctx, controlsSlot, outputs }) {
  ...
  function step() {
    ...
    if (ym <= 0 || t >= T) {
      ...
      m2Out.textContent   = `${R.toFixed(2)} m`;
      m3Out.textContent   = `${T.toFixed(2)} s`;
      draw(R, 0);

      // ★ クラウド保存（未ログインなら失敗しても無視）
      if (window.saveRunToCloud) {
        window.saveRunToCloud({
          v0: v0cur, theta: thetaDeg, h0, g,
          T, R, hMax: hMaxVal
        }).catch(err => console.warn("Cloud save failed:", err?.message || err));
      }
      return;
    }
